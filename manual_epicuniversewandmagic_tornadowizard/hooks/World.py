...
from BaseClasses import MultiWorld, CollectionState, ItemClassification # type: ignore
...
def hook_get_filler_item_name(world: World, multiworld: MultiWorld, player: int) -> str | bool:
	 ...
	 # grabbing every possible filler name
	...
	# sort the pins from the non-pins
	for filler in possible_filler_names:
		if filler.find("Pin") != -1:
			pin_fillers.append(filler)
		else:
			other_fillers.append(filler)

	# 50/50 to have a pin or something else as junk
	if world.random.randint(0, 1) == 1:
		return world.random.choice(pin_fillers)
	else:
		return world.random.choice(other_fillers)
